...
export default function HelperConversationPage() {
  ...
  // Live delivery: listen for messages:new (sent by messageController after helper/customer reply)
  // AND profile_chat:new_message (sent by chatController for first profile-initiated message)
  useEffect(() => {
    if (!socket) return;
    ...
    const handleNewMessage = (msg) => {
      ...
      setMessages(prev => {
        // Deduplicate by id in case the sender already appended optimistically
        if (prev.some(m => m.id === msg.id)) return prev;
        return [...prev, msg];
      });
    };

    // message:new — fired by messageController.sendMessage for both sides
    socket.on('message:new', handleNewMessage);

    // profile_chat:new_message — fired by chatController.profileChatMessage (live branch)
    // payload shape: { conversationId, message, senderName, senderAvatar }
    const handleProfileNew = ({ conversationId: cid, message }) => {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      // Optimistically append — socket dedup will skip if it also arrives via broadcast
      setMessages(prev => {
      ...
      // Restore unsent text so the helper doesn't lose it
