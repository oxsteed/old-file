...
const WINDOW_LABELS = { '1w':'1 Week','2w':'2 Weeks','1m':'1 Month','3m':'3 Months','6m':'6 Months','1y':'1 Year','5y':'5 Years','10y':'10 Years' };
...
const DEFAULT_FORM = {
  ...
  recurrence_interval_days:'', notes:'',
};
...
export default function PlannedNeedsPage() {
  ...
  // modal state
  const [modal, setModal] = useState(null); // null | 'add' | 'edit' | 'cancel' | 'complete'
  ...
  function openComplete(need) { setSelected(need); setForm({ actual_cost: need.estimated_cost || '' }); setModal('complete'); }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (modal === 'add') {
        await api.post('/planned-needs', payload);
      } else {
        await api.put(`/planned-needs/${selected.id}`, payload);
      }
      closeModal();
      await loadNeeds();
  ...
  async function handleComplete() {
    ...
    try {
      ...
      setError(e?.response?.data?.error || 'Failed to mark complete.');
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="pn-page">
          ...
          {projLoading ? (
            ...
            <>
              ...
              <p className="pn-proj-note">
                Income &amp; expenses based on your last 90 days of Life Pulse data.
                Sinking funds are prorated: <em>(cost − reserved) ÷ days remaining × {WINDOW_LABELS[window_]}</em>.
              </p>
            </>
          ) : (
            <p style={{ color: 'var(--muted-text)', textAlign: 'center' }}>No projection data available.</p>
          )}
        </div>
        ...
        {activatingSoon.length > 0 && (
          <div className="pn-queue-banner">
            <span>⚡</span>
            <span>
              <strong>{activatingSoon.length} need{activatingSoon.length !== 1 ? 's' : ''}</strong> will auto-publish as a job post within the next day.
              Review below and cancel if needed.
            </span>
          </div>
        )}
          ...
          <>
            ...
            {activeNeeds.length === 0 ? (
              <div className="pn-empty">
                ...
                <p>Add your first planned need — oil change, dental visit, anything coming up.</p>
              </div>
            ) : (
            ...
            <div className="pn-modal">
              ...
              <div className="pn-form-row">
                ...
                <div className="pn-form-group">
                  ...
                  <div className="pn-form-hint">Job auto-posts this many days before the due date.</div>
                </div>
              </div>
              ...
              <div className="pn-form-group">
                ...
                <textarea name="description" value={form.description} onChange={handleFormChange}
                  rows={2} placeholder="Optional — will appear in the auto-posted job." />
              </div>

              <div className="pn-form-group">
                <label>Notes (private)</label>
                <textarea name="notes" value={form.notes} onChange={handleFormChange}
                  rows={2} placeholder="Reminders or context just for you." />
              </div>
        ...
        {modal === 'cancel' && selected && (
          <div className="pn-modal-overlay" onClick={e => e.target === e.currentTarget && closeModal()}>
            <div className="pn-modal">
              ...
              <p>
                Are you sure you want to cancel <strong>{selected.title}</strong>?
                Its sinking fund contribution will be removed from future Life Pulse projections.
              </p>
        ...
        {modal === 'complete' && selected && (
          <div className="pn-modal-overlay" onClick={e => e.target === e.currentTarget && closeModal()}>
            <div className="pn-modal">
              ...
              <p>
                Marking <strong>{selected.title}</strong> as done. You can optionally record the actual cost
                — this helps improve future projections.
              </p>
              <div className="pn-form-group">
                <label>Actual Cost ($)</label>
                <input type="number" name="actual_cost" value={form.actual_cost}
                  onChange={handleFormChange} placeholder="Leave blank to keep estimate" min="0" step="0.01" />
              </div>
              {selected.recurrence_type !== 'none' && (
                <p style={{ fontSize: '0.85rem', color: 'var(--muted-text)' }}>
                  This is a recurring need — the next occurrence will be created automatically.
                </p>
              )}
              {error && <p style={{ color: '#f87171', fontSize: '0.85rem' }}>{error}</p>}
              <div className="pn-modal-actions">
    ...
    <div className={`pn-card status-${need.status}`}>
      ...
      {isActive && (
        <div className="pn-card-actions">
          {onEdit && (
            <button className="pn-card-action-btn" onClick={onEdit}>Edit</button>
          )}
          {onComplete && (
            <button className="pn-card-action-btn primary" onClick={onComplete}>Complete</button>
          )}
          {onCancel && (
            <button className="pn-card-action-btn danger" onClick={onCancel}>Cancel</button>
          )}
        </div>
      )}
    </div>
