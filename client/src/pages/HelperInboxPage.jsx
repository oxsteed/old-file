import { useState, useEffect, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import api from '../api/axios';
import { useAuth } from '../context/AuthContext';
import { useSocket } from '../hooks/useSocket';
import Navbar from '../components/Navbar';
import Footer from '../components/Footer';

export default function HelperInboxPage() {
  const [conversations, setConversations] = useState([]);
  const [loading, setLoading] = useState(true);
  // Track conversation IDs that received a new unread push via socket
  const [newInbound, setNewInbound] = useState(new Set());
  const navigate = useNavigate();
  const { user } = useAuth();
  const { socket } = useSocket();

  const fetchConversations = useCallback(async () => {
    try {
      const res = await api.get('/messages/conversations');
      setConversations(res.data);
    } catch (err) {
      console.error('[HelperInbox] Failed to load conversations:', err);
    } finally {
      setLoading(false);
    }
  }, []);

  useEffect(() => {
    fetchConversations();
  }, [fetchConversations]);

  // Listen for profile-chat messages sent TO this helper from a customer
  useEffect(() => {
    if (!socket) return;

    const handler = ({ conversationId, message, senderName }) => {
      // Refresh the list so last_message and last_message_at update
      fetchConversations();
      // Mark as newly arrived so we can highlight the row
      setNewInbound(prev => new Set(prev).add(conversationId));
    };

    socket.on('profile_chat:new_message', handler);
    return () => socket.off('profile_chat:new_message', handler);
  }, [socket, fetchConversations]);

  const handleOpen = (convId) => {
    // Clear the new-message highlight when the helper opens the thread
    setNewInbound(prev => {
      const next = new Set(prev);
      next.delete(convId);
      return next;
    });
    navigate(`/helper/messages/${convId}`);
  };

  if (loading) {
    return (
      <div className="min-h-screen flex flex-col bg-gray-50">
        <Navbar />
        <div className="flex-1 flex items-center justify-center">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600" />
        </div>
        <Footer />
      </div>
    );
  }

  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      <Navbar />
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Messages</h1>

        {conversations.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-8 text-center">
            <svg className="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
            </svg>
            <h3 className="mt-2 text-sm font-medium text-gray-900">No messages yet</h3>
            <p className="mt-1 text-sm text-gray-500">
              When a customer contacts you from your profile, their message will appear here.
            </p>
          </div>
        ) : (
          <div className="bg-white rounded-lg shadow divide-y divide-gray-200">
            {conversations.map((conv) => {
              const isNew = newInbound.has(conv.id);
              const unread = parseInt(conv.unread_count) > 0 || isNew;
              return (
                <div
                  key={conv.id}
                  onClick={() => handleOpen(conv.id)}
                  className={`p-4 hover:bg-gray-50 cursor-pointer flex items-center justify-between transition ${
                    isNew ? 'bg-blue-50' : ''
                  }`}
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <p className={`text-sm truncate ${
                        unread ? 'font-semibold text-gray-900' : 'font-medium text-gray-700'
                      }`}>
                        {conv.other_user_name}
                      </p>
                      {unread && (
                        <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                          {isNew && !parseInt(conv.unread_count) ? 'New' : conv.unread_count}
                        </span>
                      )}
                    </div>
                    {conv.job_title && (
                      <p className="text-xs text-blue-600 mb-1">Re: {conv.job_title}</p>
                    )}
                    <p className="text-sm text-gray-500 truncate">
                      {conv.last_message || 'No messages yet'}
                    </p>
                  </div>
                  <div className="ml-4 flex-shrink-0 text-xs text-gray-400">
                    {conv.last_message_at
                      ? new Date(conv.last_message_at).toLocaleDateString()
                      : ''}
                  </div>
                </div>
              );
            })}
          </div>
        )}
      </main>
      <Footer />
    </div>
  );
}
