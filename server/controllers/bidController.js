...
exports.createBid = async (req, res) => {
  try {
    ...
    if (job.rows[0].poster_id === req.user.id) return res.status(400).json({ error: 'Cannot bid on your own job' });
    if (!['open', 'bidding'].includes(job.rows[0].status)) return res.status(400).json({ error: 'Job is not accepting bids' });
    ...
    await pool.query(`UPDATE jobs SET status = 'bidding', updated_at = NOW() WHERE id = $1 AND status = 'open'`, [job_id]);
...
exports.getJobBids = async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT b.*, u.first_name as helper_name, u.avatar_url as helper_avatar, u.avg_rating as helper_rating,
       (SELECT COUNT(*) FROM jobs WHERE helper_id = b.helper_id AND status = 'completed') as jobs_completed
       FROM bids b JOIN users u ON b.helper_id = u.id WHERE b.job_id = $1 ORDER BY b.created_at DESC`,
      [req.params.job_id]
    );
...
exports.getMyBids = async (req, res) => {
  try {
    ...
    let query = `SELECT b.*, j.title as job_title, j.category as job_category, j.status as job_status, j.location_city, j.location_state
