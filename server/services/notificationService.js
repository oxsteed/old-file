...
exports.sendNotification = async ({
  ...
  try {
    ...
    const { rows: prefRows } = await db.query(`
      SELECT np.*, u.email, u.first_name,
...
async function sendEmailNotification({ to, firstName, type, title, body, action_url }) {
  try {
    const buttonText = {
...
function buildEmailTemplate({ firstName, title, body, action_url, buttonText }) {
  return `
<!DOCTYPE html>
<html>
<head>
        ...
        <table width="560" cellpadding="0" cellspacing="0"
          ...
          <tr>
            <td style="padding:36px 32px;">
              ...
              <h1 style="margin:0 0 16px;color:#111827;font-size:22px;
                          font-weight:700;line-height:1.3;">
                ${title}
              </h1>
              <p style="margin:0 0 28px;color:#4b5563;font-size:15px;
                         line-height:1.6;">
                ${body}
              </p>
