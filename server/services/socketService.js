...
function broadcastToConversationAndUser(conversationId, userId, event, data) {
  if (!io) return;
  io.to(`conv_${conversationId}`).to(`user_${userId}`).emit(event, data);
}
...
module.exports = {
  ...
  broadcastToConversationAndUser,
  broadcastToAdmins,
};
