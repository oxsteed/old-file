...
const FEES = {
  ...
  // Broker cut is separate and only applies if a Broker Helper mediates.
  TIER_3: {
 ...
 *   - OxSteed's `application_fee_amount` = 7.1% of job value (or $5 min) + broker fee.
...
exports.calculateTier3Fees = (jobValueCents, isBrokerMediated = false) => {
  ...
  // OxSteed platform fee: 7.1% or $5 minimum — the entire platform cut
  const platformFeeCents = Math.max(
    f.platform_min,
    Math.round(jobValueCents * f.platform_pct)
  );

  // Broker mediation fee: 5% (only if brokered, separate from platform fee)
  ...
  const applicationFeeCents = platformFeeCents + brokerFeeCents;
