...
exports.getPublicFeed = async ({
  ...
  if (lat && lng) {
    params.push(lat, lng, radiusMiles * 1609.34); // Convert miles to meters
    conditions.push(
      `ST_DWithin(
         j.location_point,
         ST_SetSRID(ST_MakePoint($${params.length - 1}::float,
                                 $${params.length - 2}::float), 4326)::geography,
         $${params.length}
       )`
    );
  }
  ...
  const { rows } = await db.query(
    `SELECT
       ...
       CASE WHEN $${params.length - 1}::text IS NOT NULL
            THEN ROUND(
              ST_Distance(
                j.location_point,
                ST_SetSRID(
